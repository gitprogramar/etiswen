...
/*Animation*/
program.animate=function(selector, action, time, loop){var elements=document.querySelectorAll(selector); elements.forEach(function(value, index){time=(time !=undefined ? time : 3000);if(action=='opacity'){value.style.transition='opacity ' + time + 's ease-out';}else{value.style.animation=action + ' ' + time + 'ms' + ' linear' + (loop !=undefined ? ' infinite' : '') + ' both';}value.style.opacity=1; window.setTimeout(function(){value.style.animation='';}, time, value);});};program.animateOnScroll=function(){var elements=document.querySelectorAll('[data-animate]'); elements.forEach(function(value, index){if (window.pageYOffset >=(program.getOffset(value).top - window.innerHeight) && value.style.animation==''){var attr=value.getAttribute('data-animate').split(' '); time=(attr[1] !=undefined ? attr[1] : 3000); if(attr[0]=='opacity'){value.style.transition='opacity ' + time + 's ease-out';}else{value.style.animation=attr[0] + ' ' + time + 'ms' + ' linear' + (attr[2] !=undefined ? ' infinite' : '') + ' both';}value.style.opacity=1;}});};program.getOffset=function(el){const rect=el.getBoundingClientRect(); return{left: rect.left + window.scrollX, top: rect.top + window.scrollY};};
/*Theme selector*/
