var program={};window.onload=function(){program.onload();};window.onresize=function(){program.sliderReload(); program.menuWidth(); program.menuBorderColor();};window.onscroll=function(ev){program.animateOnScroll(); /*if ((window.innerHeight + window.pageYOffset) >=document.body.offsetHeight){console.log("bottom");}*/ program.menuFixed(); program.hideChat();};program.post=function(url, data, callback, loading){if (loading==undefined) program.modalShow('loading-simple'); else if (loading !=false) program.modalShow('loading'); var xhttp=new XMLHttpRequest(); xhttp.onreadystatechange=function(){if (this.readyState==4 && this.status==200){program.modalHide('loading-simple'); program.modalHide('loading'); if (callback !=undefined){callback(program.json(this.responseText));}else{return this.responseText;}}}; xhttp.open("POST", url, true); xhttp.setRequestHeader("Content-Type", "application/json"); xhttp.send(JSON.stringify(data));};program.get=function(url, callback, loading){if (loading==undefined) program.modalShow('loading-simple'); else if (loading !=false) program.modalShow('loading'); var xhttp=new XMLHttpRequest(); xhttp.onreadystatechange=function(){if (this.readyState==4 && this.status==200){program.modalHide('loading-simple'); program.modalHide('loading'); if (callback !=undefined){callback(program.json(this.responseText));}else{return this.responseText;}}}; xhttp.open("GET", url, true); xhttp.send();};program.json=function(val){return val.indexOf("{") !=-1 ? JSON.parse(val.substring(val.indexOf("{"), val.lastIndexOf("}") + 1)) : val;};program.addStyle=function(css, id){var s=document.createElement('style'); s.type='text/css'; s.innerText=css; if (id !=undefined){s.id=id; if (document.getElementById(id) !=undefined) document.getElementById(id).remove();}document.body.appendChild(s);};program.rootGet=function(prop){return getComputedStyle(document.getElementById("theme")).getPropertyValue(prop);};program.rgbaGet=function(prop, opacity){var color=program.rootGet(prop).replace('#', ''); if (opacity==undefined) opacity='1'; return 'rgba(' + parseInt(color.substring(1, 3), 16) + ',' + parseInt(color.substring(3, 5), 16) + ',' + parseInt(color.substring(5, 7), 16) + ',' + opacity + ')';};program.scroll=function(element){document.querySelector(element).scrollIntoView({behavior: 'smooth'});};program.getParameter=function(name, url){if (!url) url=window.location.href; name=name.replace(/[\[\]]/g, "\\$&"); var regex=new RegExp("[?&]" + name + "(=([^&#]*)|&|#|$)"), results=regex.exec(url); if (!results) return null; if (!results[2]) return ''; return decodeURIComponent(results[2].replace(/\+/g, " "));};program.addCss=function(styles){/*USAGE:var styles=[];styles.push("/assets/font-awesome.min.css");window.setTimeout(program.addCss,5000,styles);*/ var head=document.head, link, exists=false; for (var x=0; x < styles.length; x++){for (var y=0; y < document.styleSheets.length; y++){if (document.styleSheets[y].href !=null && document.styleSheets[y].href.indexOf(styles[x].substr(styles[x].lastIndexOf('/') + 1)) !=-1){exists=true; break;}}if (exists){break;}link=document.createElement('link'); link.type='text/css'; link.rel='stylesheet'; link.href=styles[x]; head.appendChild(link);}}; /*Reload Sliders for css changes*/program.sliderReload=function(){var sliders=document.querySelectorAll('.slider'); for (var x=0; x < sliders.length; x++){program.slider.init(sliders[x].id, null);}};program.addScripts=function(scripts){var head=document.head, script, exists=false; for (var x=0; x < scripts.length; x++){for (var y=0; y < document.scripts.length; y++){if (document.scripts[y].src.indexOf(scripts[x].substr(scripts[x].lastIndexOf('/') + 1)) !=-1){exists=true; break;}}if (exists){break;}script=document.createElement('script'); script.type='text/javascript'; script.src=scripts[x]; head.appendChild(script);}};
/*Slider*/
program.slider={touchstartx:{}, touchmovex:{}, movex:{}, init: function(id, time){/*Video*/ var videos=document.querySelectorAll('#' + id + ' video'); var minHeight; for (var x=0; x < videos.length; x++){var videoHeight=getComputedStyle(videos[x]).height.replace('px', ''); if (minHeight==undefined || videoHeight < minHeight) minHeight=videoHeight; /*Play*/ videos[x].parentNode.addEventListener("click", function(e){this.style.zIndex=-1; this.parentNode.style.backgroundImage='none'; this.firstElementChild.play();});}/*Config*/ this.touchstartx[id]=undefined; this.touchmovex[id]=undefined; this.movex[id]=undefined; var container=document.getElementById(id); container.removeAttribute("style"); var height=getComputedStyle(container).height.replace('px', ''); if (minHeight !=undefined && height > minHeight) height=minHeight; container.style.height=height + 'px'; var slideCount=container.firstElementChild.children.length; program.addStyle('#' + id + ' > div{width: ' + (slideCount * 100) + '%;}#' + id + ' div.slide{width: ' + (100 / slideCount) + '%;}', id + '-style'); /*Dots*/ var dotContainer=document.querySelector("#" + id + " #dots").firstChild; var dots=dotContainer.children; var isActiveDot=false; for (var x=0; x < dots.length; x++){if (dots[x].classList.contains('active-dot')) isActiveDot=true; dots[x].addEventListener('click', function(e){var container=e.currentTarget.parentNode.parentNode.parentNode; var dots=document.querySelectorAll('#' + container.id + ' span[data-id]'); for (var x=0; x < dots.length; x++){dots[x].classList.remove('active-dot');}this.classList +=' active-dot'; container.firstElementChild.style.transform='translate3d(-' + this.getAttribute("data-id") * getComputedStyle(container).width.replace('px', '') + 'px,0,0)'; /*clear interval*/ var interval=container.getAttribute('data-interval'); if (interval !=undefined) window.clearInterval(interval);});}; if (!isActiveDot) dots[0].classList +=' active-dot'; /*Timmer*/ if (time !=undefined){var id=window.setInterval(this.sliderInterval, time * 1000, id); container.setAttribute('data-interval', id);}; var holder=container.firstElementChild; /*Desktop*/ holder.ondragstart=function(event){var img=document.createElement("img"); img.src='/images/drag.jpg'; if (event.dataTransfer !=undefined && event.dataTransfer.setDragImage !=undefined) event.dataTransfer.setDragImage(img, 0, 0); program.slider.start(event);}; holder.ondragover=function(event){program.slider.move(event);}; holder.ondragleave=function(event){program.slider.end(event);}; /*Mobile|*/ holder.ontouchstart=function(event){program.slider.start(event);}; holder.ontouchmove=function(event){program.slider.move(event);}; holder.ontouchend=function(event){program.slider.end(event);};}, start: function(event){/* Get the original touch position. */ var container=event.currentTarget.parentNode; this.touchstartx[container.id]=event.touches !=undefined ? event.touches[0].pageX : event.pageX; /*Refresh Index*/}, move: function(event){/* Continuously return touch position.*/ var container=event.currentTarget.parentNode; var slideWidth=getComputedStyle(container).width.replace('px', ''); var slideCount=event.currentTarget.children.length; var index=document.querySelector('#' + container.id + ' span.active-dot').getAttribute('data-id'); this.touchmovex[container.id]=event.touches !=undefined ? event.touches[0].pageX : event.pageX; /* Calculate distance to translate holder. */ this.movex[container.id]=index * slideWidth + (this.touchstartx[container.id] - this.touchmovex[container.id]); /* Defines the speed the images should move at.*/ var panx=100 - this.movex[container.id] / 6; if (this.movex[container.id] < (slideWidth * slideCount)){/* Makes the holder stop moving when there is no more content.*/ event.currentTarget.style.transform='translate3d(-' + this.movex[container.id] + 'px,0,0)';}}, end: function(event){/* Calculate the distance swiped.*/ var container=event.currentTarget.parentNode; var slideWidth=getComputedStyle(container).width.replace('px', ''); var slideCount=container.firstElementChild.children.length; var index=document.querySelector('#' + container.id + ' span.active-dot').getAttribute('data-id'); var absMove=Math.abs(index * slideWidth - this.movex[container.id]); /* Calculate the index. All other calculations are based on the index.*/ if (absMove > slideWidth / 2){if (this.movex[container.id] > index * slideWidth && index < slideCount - 1){index++;}else if (this.movex[container.id] < index * slideWidth && index > 0){index--;}program.slider.eventListener(index);}var dots=document.querySelectorAll('#' + container.id + ' span[data-id]'); for (var x=0; x < dots.length; x++){dots[x].classList.remove('active-dot');}dots[index].classList +=' active-dot'; /* Move and animate the elements.*/ event.currentTarget.style.transform='translate3d(-' + index * slideWidth + 'px,0,0)';}, sliderInterval: function(id){var container=document.getElementById(id); var activeDot=document.querySelector('#' + container.id + ' span.active-dot'); var currentIndex=parseInt(activeDot.getAttribute('data-id')) + 1; var dots=document.querySelectorAll('#' + container.id + ' span[data-id]'); for (var x=0; x < dots.length; x++){dots[x].classList.remove('active-dot');}if (currentIndex >=dots.length) currentIndex=0; dots[currentIndex].classList +=' active-dot'; container.firstElementChild.style.transform='translate3d(-' + currentIndex * getComputedStyle(container).width.replace('px', '') + 'px,0,0)'; "translate3d(-1366px,0,0)"; program.slider.eventListener(currentIndex);}, isPlaying: function(video){return video.currentTime > 0 && !video.paused && !video.ended && video.readyState > 2;}, eventListener: function(index){/*subscribe this to event when user has dragged the slider*/ /*program.slider.eventListener=function(index){console.log('drag ended');}*/}};
/*Menu*/
program.menuBind=function(){var menu=document.getElementById("cssmenu"); var divMenu=document.createElement("div"); divMenu.id="menu-button"; menu.insertBefore(divMenu, menu.firstChild); var menuBtn=document.getElementById("menu-button"); menuBtn.addEventListener('click', function(){this.classList.toggle("menu-opened"); this.parentNode.children[1].classList.toggle("open");}); program.menuPrependPlusIcons(); program.menuBorderColor();program.menuWidth(); var subMenuBtn=document.getElementsByClassName("submenu-button"); for (var x=0; x < subMenuBtn.length; x++){subMenuBtn[x].addEventListener('click', function(){this.classList.toggle("submenu-opened"); this.firstChild.classList.toggle("fa-rotate-180"); var subMenuBtnChilds=this.parentNode.childNodes; for (var y=0; y < subMenuBtnChilds.length; y++){if (subMenuBtnChilds[y].nodeName.toLowerCase()=="ul" && subMenuBtnChilds[y].classList.contains("open")){subMenuBtnChilds[y].style.display="none"; subMenuBtnChilds[y].classList.remove("open");}else if (subMenuBtnChilds[y].nodeName.toLowerCase()=="ul" && !subMenuBtnChilds[y].classList.contains("open")){subMenuBtnChilds[y].style.display="block"; subMenuBtnChilds[y].classList.toggle("open");}}});}; /*fix media query on IE*/ if (navigator.userAgent.indexOf('MSIE') !==-1 || navigator.appVersion.indexOf('Trident/') > 0){if (window.innerWidth >=1000){var wHeigth=window.outerHeight; window.resizeTo(window.outerWidth, window.innerHeight); window.setTimeout(function(){window.resizeTo(window.outerWidth, wHeigth); console.log("resized from Menu");}, 100, wHeigth);}};};program.menuPrependPlusIcons=function(){var subMenus=document.getElementsByClassName('has-sub'); var spanMenu; var icon; var menuElement; var attribute; for (var x=0; x < subMenus.length; x++){menuElement=subMenus[x]; spanMenu=document.createElement("span"); spanMenu.className="submenu-button"; icon=document.createElement("i"); icon.className="fa"; icon.className +=" fa-chevron-down"; icon.className +=" effect"; attribute=document.createAttribute("aria-hidden"); attribute.value=true; icon.setAttributeNode(attribute); spanMenu.appendChild(icon); menuElement.insertBefore(spanMenu, menuElement.firstChild);}};program.menuFixed=function(){if (window.innerWidth >=1000){/* fixed menu for desktop and tablet only*/ var nav=document.getElementById("menu"); if (window.pageYOffset==0){nav.classList.remove("menu-scrolled");}else if (!nav.classList.contains("menu-scrolled")){nav.classList.toggle("menu-scrolled");}}};program.menuBorderColor=function(){var li=document.querySelectorAll('#cssmenu li'); var span=document.querySelectorAll('#cssmenu span.submenu-button'); if (window.innerWidth <=1000){/*Border color*/ for (var x=0; x < li.length; x++){li[x].style.borderTop='1px solid ' + program.rgbaGet('--third', .4);}for (var x=0; x < span.length; x++){span[x].style.borderLeft='1px solid ' + program.rgbaGet('--third', .4);}}else{for (var x=0; x < li.length; x++){li[x].removeAttribute('style');}for (var x=0; x < span.length; x++){span[x].removeAttribute('style')}}};program.menuWidth=function(){var width=window.innerWidth;if(width<=1000){document.querySelector('#cssmenu > ul').style.width=getComputedStyle(document.getElementsByTagName('body')[0]).width;}else{document.querySelector('#cssmenu > ul').removeAttribute('style');}};
/*Modal*/
program.modalShow=function(id, position, closable){if (closable !=undefined){var close=document.getElementsByClassName("modal-close")[0]; close.style.display="block"; close.addEventListener("click", function(){program.modalHide(id);});}var element=document.getElementById(id); element.classList.toggle("modal-hidden"); element.style.top=(window.innerHeight / (position==undefined ? 2 : position)) + window.pageYOffset + "px"; element.style.zIndex=10000; element.style.opacity=1; if (id !='loading-simple') document.getElementsByTagName("html")[0].style.overflow="hidden";};program.modalHide=function(id){document.getElementById(id).style.opacity=0; window.setTimeout(function(){hide(id);}, 700);};function hide(id){var element=document.getElementById(id); element.classList.toggle("modal-hidden"); var close=document.getElementsByClassName("modal-close")[0]; close.style.display="none"; element.style.zIndex=-10000; document.getElementsByTagName("html")[0].style.overflowY="scroll";};
/*Notify*/
program.notify=function(pTitle, pMsg, pType, pTime){var notify=document.getElementById("notify"); if (notify.classList.contains("modal-hidden")){notify.classList.toggle("modal-hidden");}if (notify.classList.contains("notify-show")){notify.classList.toggle("notify-show"); notify.classList.toggle("notify-discard");}if (notify.classList.contains("notify-success")){notify.classList.toggle("notify-success");}if (notify.classList.contains("notify-fail")){notify.classList.toggle("notify-fail");}if (notify.classList.contains("notify-warning")){notify.classList.toggle("notify-warning");}if (notify.classList.contains("notify-theme")){notify.classList.toggle("notify-theme");}notify.classList.toggle("notify-" + pType); document.getElementById("notify-title").innerHTML=pTitle; document.getElementById("notify-message").innerHTML=pMsg; notify.classList.toggle("notify-show"); notify.style.top=(window.pageYOffset + 30) + "px"; if (pTime==undefined){pTime=6000;}window.setTimeout(function(){var notify=document.getElementById("notify"); if (notify.classList.contains("notify-discard")){notify.classList.toggle("notify-discard");}else{notify.classList.toggle("notify-show"); notify.classList.toggle("modal-hidden");}}, pTime);};program.notifyHide=function(){var notify=document.getElementById("notify");if (notify.classList.contains("notify-show")){notify.classList.toggle("notify-show"); notify.classList.toggle("notify-discard");}};
/*Cookies*/
program.Cookies={};program.Cookies.get=function(name){var v=document.cookie.match('(^|;) ?' + name + '=([^;]*)(;|$)'); return v ? v[2] : null;};program.Cookies.set=function(name, value, days){var d=new Date;if(days==undefined)days=30; d.setTime(d.getTime() + 24*60*60*1000*days); document.cookie=name + "=" + value + ";path=/;expires=" + d.toGMTString();};program.Cookies.delete=function(name){program.Cookies.set(name, '', -1);};
/*Animation*/
program.animate=function(objectOrSelector, action, time, loop){var elements=[];if(typeof objectOrSelector=='string')elements=document.querySelectorAll(objectOrSelector);else if(Array.isArray(objectOrSelector))elements=objectOrSelector;else{elements.push(objectOrSelector);}elements.forEach(function(value, index){time=(time !=undefined ? time : 3000); if (action=='opacity'){value.style.transition='opacity ' + time + 's ease-out';}else{value.style.animation=action + ' ' + time + 'ms' + ' linear' + (loop !=undefined ? ' infinite' : '') + ' both';}value.style.opacity=1; window.setTimeout(function(){value.style.animation='';}, time, value);});};program.animateOnScroll=function(){var elements=document.querySelectorAll('[data-animate]'); elements.forEach(function(value, index){if (window.pageYOffset >=(program.getOffset(value).top - window.innerHeight) && value.style.animation==''){var attr=value.getAttribute('data-animate').split(' '); time=(attr[1] !=undefined ? attr[1] : 3000); if (attr[0]=='opacity'){value.style.transition='opacity ' + time + 's ease-out';}else{value.style.animation=attr[0] + ' ' + time + 'ms' + ' linear' + (attr[2] !=undefined ? ' infinite' : '') + ' both';}value.style.opacity=1;}});};program.getOffset=function(el){const rect=el.getBoundingClientRect(); return{left: rect.left + window.scrollX, top: rect.top + window.scrollY};};
/*Theme selector*/
program.themeSet = function(themeId) {
    program.addStyle('* { 	transition: color 1s ease-out !important; }  main, div, .btn { 	transition: background 1s ease-out !important;}', 'themetransition');
    program.modalShow("loading-simple");
    program.post("/api/post.php", {
        action: "themeGet",
        themeId: themeId
    }, function(response) {
        document.querySelector("#theme").innerHTML = ':root{--white: #ffffff; --black: #29293a; --first: ' + response.value.first + '; --second:  ' + response.value.second + '; --third:  ' + response.value.third + '; --menu:  ' + response.value.menu + '; --bgheader:  ' + response.value.bgheader + '; --bgbody: ' + response.value.bgbody + '; --bgfooter: ' + response.value.bgfooter + '; --extra:  ' + response.value.extra + '}';
        window.setTimeout(function() {
            program.modalHide("loading-simple");
			program.sliderReload();
            document.getElementById("themetransition").remove();
        }, 1200);
    });
};
program.hideChat = function() {
	if (typeof(FB) != "undefined" && (window.innerHeight + window.pageYOffset) >=document.body.offsetHeight)
		FB.CustomerChat.hideDialog();	
};
program.onload = function() {
    program.menuBind();
	program.animateOnScroll();
    /*year*/
	var year=document.querySelector("#copyright #year");if(year !=undefined){year.innerHTML=new Date().getFullYear();/*panel*/year.addEventListener('click', function(e){e.preventDefault();window.open('/panel', "_blank");});/*session*/document.getElementById("clearsession").addEventListener('click', function(e){e.preventDefault();program.post("/api/post.php",{action: "sessionClear"}, function(response){console.log(response.value);});});}
	/*whatsapp*/
	var whatsapp=document.querySelectorAll(".whatsapp");var whatsapp2=document.querySelectorAll(".whatsapp2");var message=encodeURIComponent(customer.customername + ' ' + customer.message);var subject=encodeURIComponent(customer.subject + ' - ' + customer.customername);if(window.innerWidth >=745){for(var x=0; x<whatsapp.length; x++){whatsapp[x].href='//api.whatsapp.com/send?phone='+customer.celParsed+'&text='+(whatsapp[x].getAttribute('data-value')==undefined ? message : encodeURIComponent(whatsapp[x].getAttribute('data-value'))); whatsapp[x].target="_blank";}for(var x=0; x<whatsapp2.length; x++){whatsapp2[x].href='//api.whatsapp.com/send?phone='+customer.cel2Parsed+'&text='+(whatsapp2[x].getAttribute('data-value')==undefined ? message : encodeURIComponent(whatsapp2[x].getAttribute('data-value'))); whatsapp2[x].target="_blank";}}else{for(var x=0; x<whatsapp.length; x++){whatsapp[x].href='//wa.me/+'+customer.celParsed+'?text='+(whatsapp[x].getAttribute('data-value')==undefined ? message : encodeURIComponent(whatsapp[x].getAttribute('data-value')));}for(var x=0; x<whatsapp2.length; x++){whatsapp2[x].href='//wa.me/+'+customer.cel2Parsed+'?text='+(whatsapp2[x].getAttribute('data-value')==undefined ? message : encodeURIComponent(whatsapp2[x].getAttribute('data-value')));}}
	/*mailto*/
	var mailto=document.querySelectorAll(".mailto");for(var x=0; x<mailto.length; x++){mailto[x].href='mailto:'+customer.email+'?subject='+subject+'&body='+(mailto[x].getAttribute('data-value')==undefined ? message : encodeURIComponent(mailto[x].getAttribute('data-value')));mailto[x].innerHTML +=customer.email;}
    /*Fix index.php links*/
    var links=document.querySelectorAll('a[href*="index.php"]'); for (var x=0; x < links.length; x++){links[x].href=links[x].href.replace("index.php", "");}
	/*combo*/
	var combos=document.querySelectorAll('div.combo > a');for(var x=0;x<combos.length; x++){combos[x].addEventListener('click', function(e){this.nextElementSibling.children[0].classList.toggle("combo-active");});}document.onclick=function(e){if(e.target.parentNode !=undefined && !e.target.parentNode.classList.contains('combo')){var combos=document.querySelectorAll('div.combo > a'); for(var x=0;x<combos.length; x++){if(combos[x].nextElementSibling.children[0].classList.contains('combo-active')) combos[x].nextElementSibling.children[0].classList.remove("combo-active");}}};
	/*messenger*/
	if(typeof messenger === "function") {window.setTimeout(messenger(),2000);}
};
