...
class ContentModelArticle extends JModelAdmin
{
	...
	public function save($data)
	{
		...
		// format HTML
		$articleParsed;
		$formValues = $input->post->get('jform', array(), 'array');
		foreach($formValues as $key => $value) {
			// title alias articletext state catid featured access language version_note images urls attribs publish_up publish_down created created_by created_by_alias modified version hits id metadesc metakey xreference metadata
			if($key == 'articletext')
				$articleParsed = preg_replace('/\s+/S', ' ', str_replace(array("// <![CDATA[", "// ]]>"), array("", ""), $value));
		}

		$data['introtext'] = $articleParsed;
		//$data['introtext'] = '<p>my test save!</p>';
		if (parent::save($data))
		{
