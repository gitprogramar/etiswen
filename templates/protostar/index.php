...
/*Large*/@media screen and (min-width:1000px){body{font-size:22px;}.videoContainer{width:80%;}}/*Landscape*/@media screen and (max-width:1200px){body{font-size:20px;}}/*Medium*/@media screen and (max-width:1000px){body{font-size:17px;}#form > div{flex-direction:column;}#form input,#form textarea,#form button{width:80% !important;}#form button{width:100%;}}/*Small*/@media screen and (max-width:600px){body{font-size:15px;}.page-header{padding:8% 4% 0%;}div[itemprop="articleBody"] > div,#form > div,#header-content > div {flex-direction:column;}div[itemprop="articleBody"] > div > div{width:100%;margin:4% 0%;} .responsive { padding-top:10%;padding-bottom:10%;display:flex;flex-direction:column;justify-content:center;align-items:center !important;} .responsive > * { width:90% !important;} .responsive p { text-align:center;} #notify{max-width:50%;}#form input,#form textarea{width:80% !important;}#form button{width:100%;}.change-order-1{order:1;}.change-order-2{order:2;}.change-order-3{order:3;}.change-order-4{order:4;}}
	/*Cross-browser Flexbox Model CSS*/.column-center{display:flex;flex-direction:column;justify-content:center;align-items:center;} .column-vertical { height:inherit;} .column-space-around{display:flex;flex-direction:column;justify-content:space-around;align-items:center;}.column-space-between{display:flex;flex-direction:column;justify-content:space-between;align-items:center;}.row-center{display:flex;justify-content:center;align-items:center;}.row-space-between{display:flex;justify-content:space-between;align-items:center;}.row-space-around{display:flex;justify-content:space-around;align-items:center;}.flex{display:flex;}.j-center{justify-content:center;}.j-start{justify-content:flex-start;}.j-between{justify-content:space-between;}.j-around{justify-content:space-around;}.a-center{align-items:center;}.a-start{align-items:flex-start;}.d-column{flex-direction:column;}.d-row{flex-direction:row;}.o-1{order:1;}.w-10{width:10%;}.w-15{width:15%;}.w-20{width:20%;}.w-23{width:23%;}.w-25{width:25%;}.w-30{width:30%;}.w-40{width:40%;}.w-45{width:45%;}.w-50{width:50%;}.w-55{width:55%;}.w-60{width:60%;}.w-65{width:65%;}.w-70{width:70%;}.w-80{width:80%;}.w-90{width:90%;}.w-100{width:100%;}.column-pad *:not(a):not(svg),#searchForm > div{margin-bottom:10px;}.no-column-pad{margin-bottom:0px !important;}
</style>
<!--Common js-->
<script type="text/javascript">
program.getAsync=function(module,position,callback){var xhttp=new XMLHttpRequest();xhttp.onreadystatechange=function(){if (this.readyState==4 && this.status==200){var json=JSON.parse(this.responseText);var pos=document.getElementById(isNaN(position.substr(position.length - 2)) ? position.substr(0,position.length - 1) :position.substr(0,position.length - 2));/*removes last character (templateId)*/ if (json.css !=undefined) pos.innerHTML='<style>' + json.css + '</style>';if (json.html !=undefined) pos.innerHTML +=json.html;eval(json.js);if (callback !=undefined){callback();}}};xhttp.open("GET","/async/module.php?type=" + module + "&position=" + position + "&menuid=" + program.menuid,true);xhttp.send();};program.addCss=function(styles){var head=document.head,link,exists=false;for (var x=0;x < styles.length;x++){for (var y=0;y < document.styleSheets.length;y++){if (document.styleSheets[y].href !=null && document.styleSheets[y].href.indexOf(styles[x].substr(styles[x].lastIndexOf('/') + 1)) !=-1){exists=true;break;}}if (exists){break;}link=document.createElement('link');link.type='text/css';link.rel='stylesheet';link.href=styles[x];head.appendChild(link);}};program.addScripts=function(scripts){var head=document.head,script,exists=false;for (var x=0;x < scripts.length;x++){for (var y=0;y < document.scripts.length;y++){if (document.scripts[y].src.indexOf(scripts[x].substr(scripts[x].lastIndexOf('/') + 1)) !=-1){exists=true;break;}}if (exists){break;}script=document.createElement('script');script.type='text/javascript';script.src=scripts[x];head.appendChild(script);}};program.getParameter=function(name,url){if (!url) url=window.location.href;name=name.replace(/[\[\]]/g,"\\$&");var regex=new RegExp("[?&]" + name + "(=([^&#]*)|&|#|$)"),results=regex.exec(url);if (!results) return null;if (!results[2]) return '';return decodeURIComponent(results[2].replace(/\+/g," "));};program.scroll=function(){var elementPosition=function(a){return function(){return a.getBoundingClientRect().top;};};var scrolling=function(elementID){var el=document.getElementById(elementID),elPos=elementPosition(el),duration=400,increment=Math.round(Math.abs(elPos()) / 40),time=Math.round(duration / increment),prev=0,E;function scroller(){E=elPos();if (E===prev){return;}else{prev=E;}increment=(E > -20 && E < 20) ? ((E > -5 && E < 5) ? 1 :5) :increment;if (E > 1 || E < -1){if (E < 0){window.scrollBy(0,-increment);}else{window.scrollBy(0,increment);}setTimeout(scroller,time);}}scroller();};return{To:scrolling}};window.onload=function(){program.getAsync("menu","header-top" + program.templateId,menuLoaded);function menuLoaded(){window.setTimeout(function(){document.getElementById("header-top").classList.toggle("header-top-load");},600);if (window.innerWidth >=900 || program.showOnMobile){program.getAsync("html","header-content" + program.templateId,headerContentCallback);}else{/*window.setTimeout(function(){program.getAsync("html","header-content",headerContentCallback);},5000);*/}if (document.getElementById("showChildPages") !=null){bindChildPage();}if (document.getElementById("showSiteMap") !=null){bindSiteMap();}program.getAsync("html","footer-top" + program.templateId,footerTopCallback);}function headerContentCallback(){window.setTimeout(function(){document.getElementById("header-content").classList.toggle("header-content-load");},300);}function footerTopCallback(){window.setTimeout(function(){var footerTop=document.getElementById("footer-top");if (footerTop !=undefined){document.getElementById("footer-top").classList.toggle("footer-top-load");}},500);program.getAsync("html","footer-content" + program.templateId,footerContentCallback);}function footerContentCallback(){window.setTimeout(function(){document.getElementById("footer-content").classList.toggle("footer-content-load");},600);}/*var styles=[];styles.push("/assets/font-awesome.min.css");window.setTimeout(program.addCss,5000,styles);*/};/*window.onscroll=function(ev){if ((window.innerHeight + window.pageYOffset) >=document.body.offsetHeight){console.log("bottom");}};*/function bindChildPage(){var html='<ul style="padding-top:10px;">';for (var x=0;x < program.jsonMenu.length;x++){if (program.jsonMenu[x].parent_id==1 && program.jsonMenu[x].title.toLowerCase()==document.title.toLowerCase()){html +='<li>';if (program.jsonMenu[x].home=="1"){program.jsonMenu[x].route="";}html +='<a href="/' + program.jsonMenu[x].route + '">' + program.jsonMenu[x].title + '</a>';html +=recursiveBindChilds(program.jsonMenu,program.jsonMenu[x].id);html +='</li>';}}document.getElementById("showChildPages").innerHTML=html;};function recursiveBindChilds(items,parentId){var hasChilds=false;var html="";for (var x=0;x < items.length;x++){if (items[x].parent_id==parentId){if (!hasChilds){html +='<ul style="padding-top:10px;">';hasChilds=true;}html +='<li>';html +='<a href="/' + items[x].route + '">' + items[x].title + '</a>';html +=recursiveBindChilds(items,items[x].id);html +='</li>';}}if (hasChilds){html +='</ul>';}return html;};
</script>
</head>
<body>
<!--JSON-LD-->
